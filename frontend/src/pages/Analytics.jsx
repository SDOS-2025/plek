...
function Analytics() {
      ...
      try {
        ...
        switch (activeTab) {
          ...
          case "topUsers":
            ...
            response = {
              data: [
                ...
                { name: "Alice Johnson", department: "Marketing", bookings: 12 },
          ...
          case "mostBookedRooms":
            ...
            response = {
              data: [
                { room: "B512", building: "R&D Building", bookings: 23, capacity: 8 },
                { room: "A204", building: "Main Building", bookings: 19, capacity: 12 },
                { room: "C101", building: "Marketing Building", bookings: 17, capacity: 6 },
                { room: "D305", building: "Executive Building", bookings: 14, capacity: 16 }
              ]
            };
            break;
          ...
          case "leastBookedRooms":
            ...
            response = {
              data: [
                { room: "E201", building: "Annex Building", bookings: 2, capacity: 4 },
                { room: "F102", building: "Remote Office", bookings: 3, capacity: 8 },
                { room: "G305", building: "Research Building", bookings: 5, capacity: 10 },
                { room: "H401", building: "Training Center", bookings: 6, capacity: 20 }
              ]
            };
            break;
          ...
          case "utilization":
            ...
            response = {
              data: [
                { room: "B512", building: "R&D Building", utilization: 85, capacity: 8 },
                { room: "A204", building: "Main Building", utilization: 72, capacity: 12 },
                { room: "C101", building: "Marketing Building", utilization: 64, capacity: 6 },
                { room: "D305", building: "Executive Building", utilization: 45, capacity: 16 }
              ]
            };
            break;
  ...
  const renderTabContent = () => {
    ...
    if (data.length === 0) {
      return (
        <div className="bg-plek-dark rounded-lg p-8 text-center">
          <p className="text-gray-400">No data available for the selected category.</p>
        </div>
      );
    }
    ...
    switch (activeTab) {
          ...
          <div className="space-y-6">
            <div className="bg-plek-dark rounded-lg p-6 shadow-card">
                ...
                <div className="h-full flex items-end justify-between">
                  {data.map((item, index) => (
                    <div key={index} className="flex flex-col items-center space-y-2 w-1/6">
                      ...
                      ></div>
                      <div className="text-xs text-gray-400 truncate">{item.month}</div>
                    </div>
                  ))}
                </div>
              </div>
              <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
                {data.map((item, index) => (
                  <div key={index} className="bg-plek-lightgray/50 p-4 rounded-lg">
                    <h4 className="text-sm font-medium text-gray-300">{item.month}</h4>
                    <p className="text-xl font-bold mt-1">{item.count} <span className="text-xs font-normal text-gray-400">bookings</span></p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="bg-plek-dark rounded-lg p-6 shadow-card">
              ...
              <div className="h-64 p-4">
                ...
                <div className="h-full flex items-end justify-between">
                  {data.map((item, index) => (
                    <div key={index} className="flex flex-col items-center space-y-2 w-1/12">
                      <div 
                        className="w-full bg-blue-500 rounded-t-md transition-all duration-500" 
                        style={{ height: `${(item.count / Math.max(...data.map(d => d.count))) * 100}%` }}
                      ></div>
                      <div className="text-xs text-gray-400 truncate rotate-45 origin-top-left mt-4 ml-4">{item.hour}</div>
                    </div>
                  ))}
                </div>
              </div>
              <div className="mt-8 grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-blue-900/30 p-4 rounded-lg col-span-2 lg:col-span-4">
                  <div className="flex justify-between items-center">
                    ...
                    <p className="text-xl font-bold">{data.reduce((prev, current) => (prev.count > current.count) ? prev : current).hour}</p>
                  </div>
                </div>
                {data.slice(0, 4).map((item, index) => (
                  <div key={index} className="bg-plek-lightgray/50 p-4 rounded-lg">
                    <h4 className="text-sm font-medium text-gray-300">{item.hour}</h4>
                    <p className="text-xl font-bold mt-1">{item.count} <span className="text-xs font-normal text-gray-400">bookings</span></p>
                  </div>
                ))}
              </div>
              ...
              <table className="min-w-full divide-y divide-gray-700">
                <thead className="bg-plek-lightgray/50">
                  <tr>
                    ...
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Bookings</th>
                  </tr>
                </thead>
                <tbody className="bg-plek-lightgray/20 divide-y divide-gray-700">
                  {data.map((user, index) => (
                    <tr key={index} className={index % 2 === 0 ? 'bg-plek-background/50' : ''}>
                        ...
                        <div className="flex items-center">
                          ...
                          <div className="ml-2 h-2 bg-plek-dark rounded-full w-24">
                            <div 
                              ...
                              style={{ width: `${(user.bookings / Math.max(...data.map(u => u.bookings))) * 100}%` }}
                            ></div>
                          </div>
                        </div>
                      </td>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {data.map((room, index) => (
                  <div key={index} className="bg-plek-lightgray/30 p-5 rounded-lg flex items-start gap-4">
                    <div className={`flex-shrink-0 p-3 rounded-lg ${isLeast ? 'bg-yellow-900/30' : 'bg-plek-purple/30'}`}>
                      <Building size={24} className={isLeast ? 'text-yellow-500' : 'text-plek-purple'} />
                    </div>
                    <div className="flex-grow">
                      <div className="flex justify-between items-start mb-2">
                        <h4 className="font-medium text-white">Room {room.room}</h4>
                        <span className={`px-2 py-1 text-xs font-bold rounded-full ${isLeast ? 'bg-yellow-900/30 text-yellow-400' : 'bg-purple-900/30 text-purple-400'}`}>
                          {room.bookings} bookings
                        </span>
                      </div>
                      <p className="text-sm text-gray-400 mb-3">{room.building}</p>
                      <div className="flex justify-between items-center text-xs text-gray-400">
                        <span>Capacity: {room.capacity}</span>
                        <span>Avg. usage: {Math.round(room.bookings / 30 * 100)}%</span>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div className="space-y-6">
            <div className="bg-plek-dark rounded-lg p-6 shadow-card">
                  ...
                  <div className="flex items-center">
                    <div className="relative w-full h-6 bg-plek-background rounded-full overflow-hidden">
                      <div 
                        ...
                        style={{ width: `${data.reduce((acc, curr) => acc + curr.utilization, 0) / data.length}%` }}
                      ></div>
                    </div>
                    <span className="ml-3 text-xl font-bold">{Math.round(data.reduce((acc, curr) => acc + curr.utilization, 0) / data.length)}%</span>
                  </div>
                </div>
                <div className="bg-plek-lightgray/30 p-4 rounded-lg">
                  <h4 className="text-sm font-medium text-gray-300 mb-3">Highest Utilized Room</h4>
                  <div className="flex justify-between items-center">
                    <p className="text-white font-medium">Room {data.reduce((prev, current) => (prev.utilization > current.utilization) ? prev : current).room}</p>
                    <span className="text-xl font-bold">{Math.max(...data.map(room => room.utilization))}%</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4 mt-6">
                {data.map((room, index) => (
                  <div key={index} className="bg-plek-lightgray/30 p-4 rounded-lg">
                    <div className="flex justify-between mb-2">
                      <div>
                        <h4 className="font-medium">Room {room.room}</h4>
                        <p className="text-xs text-gray-400">{room.building} • Capacity: {room.capacity}</p>
                      </div>
                      <span 
                        className={`px-2 py-1 text-xs font-medium rounded-full ${
                          room.utilization > 75 ? 'bg-green-900/30 text-green-400' : 
                          room.utilization > 50 ? 'bg-yellow-900/30 text-yellow-400' : 
                    ...
                    <div className="relative w-full h-2 bg-plek-background rounded-full overflow-hidden">
                      <div 
                        className={`absolute top-0 left-0 h-full rounded-full ${
                          room.utilization > 75 ? 'bg-green-500' : 
      ...
      default:
        return <p className='text-gray-400'>No visualization available for this data.</p>;
    }
  };
