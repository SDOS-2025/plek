...
class AnalyticsTests(TestCase):
    ...
    def test_top_users(self):
        ...
        self.assertEqual(
            response.data[0]["label"], self.user.email
        )  # Assuming email is used instead of username
        self.assertIn("label", response.data[0])
        self.assertIn("value", response.data[0])
    ...
    def test_pending_bookings(self):
        ...
        self.assertTrue(
            any(stat["label"] == self.room1.name and stat["value"] == 1 for stat in response.data)
        )
